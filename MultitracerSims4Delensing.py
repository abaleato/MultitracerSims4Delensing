...
signal_path = '/global/cscratch1/sd/engelen/simsS1516_v0.4/data/' #Location of input phi alms
spectra_path = '/global/u1/a/ab2368/correlated_tracers/'
output_path = '.'

simid = 0 #int(sys.argv[1])
...
input_phi_alm = read_phi_alms(simid,signal_path)
input_phi_map = hp.alm2map(input_phi_alm.astype('complex'), nside=nside_out)
input_phi_alm_shortened = hp.map2alm(input_phi_map, lmax=lmax)
input_kappa_alm = hp.almxfl(input_phi_alm_shortened, (1.0/2)*ells**2)
...
np.save(output_path+'combined_phi_alms_simid'+str(simid))
